...
public class ArmaCamino{
    ...
    public boolean enCiudadUniversitaria(LatLng posicion){
        LatLng limiteInfIzquierdo = new LatLng(-31.640776, -60.678170); // -31.641034,  -60.674534 <-- original
        LatLng limiteSupDerecho = new LatLng(-31.636934, -60.669457); // -31.639295,-60.670215 <-- original
        boolean esta = true;
    ...
    //Funcion para generar un vector con las locaciones del predio
    public Vector<Punto> verLocacionesPredio(){
        Vector<Punto> locaciones = new Vector<>();
        for(int i=0;i<Nodos.size();i++){
            if(Nodos.elementAt(i).getEdificio().equals("Predio")){  //equals -> contains
                locaciones.add(Nodos.elementAt(i));
            }
        }
        return locaciones;
    }
